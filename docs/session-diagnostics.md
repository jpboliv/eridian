...
The scanner is invoked only by optional stats, never by statusline refresh. Completed
assistant text is not copied into its cache; incomplete trailing JSONL bytes are
retained until their record completes. Clear the separate diagnostics cache
